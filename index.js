...
let m=["a","an","and","as","at","but","by","en","for","if","in","nor","of","on","or","per","the","to","v","v.","vs","vs.","via"],p=0,a=(c)=>c.toUpperCase()
return s.replace(/[^\s\-]+/g,(x)=>{p++;if(/[A-Z@\/]/.test(x)||m.includes(x)&&p!=1)return x;if(/:$/.test(x)) p=0;return x.replace(/^[^\w]*\w/,a);}).replace(/[^\s\-]+$/g,(x)=>x.replace(/^./,a))
}
